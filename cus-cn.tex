...
\sys_if_shell_unrestricted:T
  {
    \usepackage{texhigh}
    \THSetCS[]{cus}{\mbox{\THcolor{blue!80!black}\bfseries
      \transparent{0.7}\texhigh@underline{\transparent{1}#1#2}}}
    \THSetFallback{cs}{cus.l}{cus, latex}
    \THSetFallback{cs}{cus.r}{cus, latex}
    \THSetFallback{cs}{cus.more}{cus, latex}
    \THSetFallback{cs}{ekeys}{cus, latex}
    \THSetFallback{cs}{fmulticol}{cus, latex}
...
\subsection{多栏文字}\label{sec:fmulticol}
...
\begin{function}{\startmulticolumns,\stopmulticolumns}
  \begin{syntax}
    \verb|\startmulticolumns| \oarg{fmulticol key-val}
    ~~<content>
...
\begin{keyval}[path=fmulticol]{columns,cols}
  \begin{syntax}
    columns|cols = \marg{整数表达式} & 2
  \end{syntax}
设置多栏数。也可不必写出键名，只写数字。

可用的栏数为 1--20，如果 \keyref[fmulticol]{adj} 为
\opt{true}，则可用的栏数为 1--10。
\end{keyval}

\begin{keyval}[path=fmulticol]{outer-sep}
  \begin{syntax}
...
\begin{keyval}[path=fmulticol]{column-sep,sep}
  \begin{syntax}
...
\begin{keyval}[path=fmulticol]{first-minimal,last-minimal}
  \begin{syntax}
...
\begin{keyval}[path=fmulticol]{heading}
  \begin{syntax}
...
\begin{keyval}[path=fmulticol]{rule-width,rule-color}
  \begin{syntax}
...
\begin{keyval}[path=fmulticol]{flush,aligned,ragged,not-aligned}
  \begin{syntax}
    flush |aligned &&
    ragged|not-aligned &&
  \end{syntax}
...
\begin{keyval}[path=fmulticol]{balanced,not-balanced}
  \begin{syntax}
    balanced = <&\TTF> & true 
    not-balanced &&
  \end{syntax}
...
\begin{keyval}[path=fmulticol]{swap-column,enable-swap-column,disable-swap-column}
  \begin{syntax}
...
\begin{keyval}[path=fmulticol]{framed,framed-options,framed-options+}
  \begin{syntax}
    framed = <&none|fbox|...>
...
\cusmodule{tcb} 库提供 \opt{tcbox} 值（\cref{sec:fmulticol/framed=tcbox}），
\cusmodule{box} 库提供 \opt{lfbox} 值（\cref{sec:fmulticol/framed=lfbox}）。

使用此选项时，最好把 \keyref[fmulticol]{overflow} 设置为 \texttt{0pt}，否则可能无法正常分页。
\end{keyval}

\begin{keyval}[path=fmulticol]{adj,adj*,adj-inner,adj-outer,shorten,widen}
  \begin{syntax}
    adj  = <&\TTF> & false 
    adj* &&
    adj-inner = \marg{内侧长度}
...
\opt{adj*} 设置 \verb|adj=true| 及 \keyref[fmulticol]{not-balanced}。
...
\begin{keyval}[path=fmulticol]{wrap-box}
  \begin{syntax}
    wrap-box = <&\TTF> & false
  \end{syntax}
...
\begin{keyval}[path=fmulticol]{addto-baselineskip}
  \begin{syntax}
...
\begin{keyval}[path=fmulticol]{tolerance,pretolerance}
  \begin{syntax}
...
\begin{keyval}[path=fmulticol]{collectmore,minrows,unbalance,column-badness,final-column-badness}
  \begin{syntax}
...
\begin{keyval}[path=fmulticol]{top-fuzz,bottom-fuzz}
  \begin{syntax}
...
\begin{keyval}[path=fmulticol]{v-fuzz,h-fuzz}
  \begin{syntax}
    v-fuzz = \marg{length}
  \end{syntax}
...
\begin{keyval}[path=fmulticol]{overflow}
  \begin{syntax}
...
\begin{keyval}[path=fmulticol]{left-to-right,LR,right-to-left,RL}
  \begin{syntax}
    left-to-right|LR &&
    right-to-left|RL &&
  \end{syntax}
...
\begin{function}{\startextrawidth,\stopextrawidth}
  \begin{syntax}
    ...
    \V\startextrawidth \oarg{fmulticol key-val} \marg{inner extra width} \marg{outer extra width}
    ~~\meta{material}
    \V\stopextrawidth
  \end{syntax}
...
\begin{function}{\startfullpagewidth,\stopfullpagewidth}
  \begin{syntax}
    ...
    \V\startfullpagewidth \oarg{fmulticol} \string<\meta{inner shrink},\meta{outer shrink}\string>
    ~~<material>
    \V\stopfullpagewidth
  \end{syntax}
...
\begin{keyval}[path=title/...]{write-tocline}
  \begin{syntax}
    write-tocline = \TTF & true
  \end{syntax}
指定是否写入目录及书签。
\end{keyval}
...
\begin{syntax}
  \V\tableofcontents      \oarg{fmulticol keyval}
  \V\localtableofcontents \oarg{fmulticol keyval} \string(<level>,<index>\string)
\end{syntax}
...
\begin{function}{\multicolplaincombinedlist}
  \begin{syntax}
    \verb|\multicolplaincombinedlist| \oarg{fmulticol key-val} \marg{title} \marg{cbl type}
  \end{syntax}
\cs{multicolplaincombinedlist} 输出 plain 格式的多栏目录，该目录的类型是 \meta{cbl type}，
并以 \meta{title} 作为标题。如果 \meta{title} 为空，则不输出标题。
\meta{fmulticol key-val} 设置多栏选项，如果栏数为 1，则相当于默认的单栏目录。
\end{function}

\begin{function}{\multicollocalplaincombinedlist}
  \begin{syntax}
    \V\multicollocalplaincombinedlist \oarg{fmulticol keyval}
...
\begin{function}{\templatetoc,\templatelof,\templatelot}
  \begin{syntax}
    \V\templatetoc
    \V\templatetoc \oarg{templatecbl keys} \oarg{fmulticol key} \string(\meta{level},\meta{index}\string)
  \end{syntax}
...
\begin{function}{\templatecombinedlist}
  \begin{syntax}
    ...
    \V\templatecombinedlist \oarg{templatecbl keys} \oarg{fmulticol key} \marg{cbl type} 
...
在不能使用浮动体的地方也无法使用这个命令，如在一个盒子中使用它们时无效。
\end{function}
...
\texttt{last-column} 分别代表最左列和最右列。以及 \texttt{mcol-\meta{i}} 用于 \pkg{multicol} 宏包的个栏。
...
\veta{templatecbl keys}、\veta{fmulticol keys}、目录自动添加的章节标题
...
\begin{itemize}[nosep]
  ...
  \item \texttt{multi}：多栏选项（\meta{fmulticol key-val}）；
...
\href{https://github.com/Sophanatprime/cindex}{cindex} 是一个新的索引制作工具，它除了基本兼容 \pkg{makeindex} 外，还支持使用 \texttt{lua} 进一步定制。同时也支持一种 \texttt{ikv} 格式的索引输入文件。本库为这一格式提供了支持。

\begin{function}{\kvindex}
  \begin{syntax}
    \V\kvindex \oarg{index type} \marg{index-kv options}
  \end{syntax}
写入一个 \texttt{ikv} 格式的索引条目。索引的内容由 \meta{index-kv options} 指定。
\end{function}

\begin{keyval}[path=index-kv]{page}
  \begin{syntax}
    page = \marg{page}
  \end{syntax}
设置索引条目的页码，可以是多级页码。会在合适的地方展开，因此，某些脆弱的命令可能需要用 \tn{protect} 保护起来。
\end{keyval}

\begin{keyval}[path=index-kv]{page-commands}
  \begin{syntax}
    page-commands = \marg{page-commands}
  \end{syntax}
设置显示页码所需的命令。会在合适的地方展开，因此，某些脆弱的命令可能需要用 \tn{protect} 保护起来。
\end{keyval}

\begin{keyval}[path=index-kv]{
  key1,display1,
  key2,display2,
  key3,display3,
  key4,display4,
  ...
  \begin{syntax}
    key1     = \marg{sort key}
    display1 = \marg{display text}
  \end{syntax}
设置对应级别的排序键以及显示文本。从 1 级开始，最多 5 级，级别之间不能断开，否则可能显示不全。
若某个 \texttt{key} 为空，则认为它等于对应级别的 \texttt{display}。

会在合适的地方展开，因此，某些脆弱的命令可能需要用 \tn{protect} 保护起来。
\end{keyval}

\begin{keyval}[path=index-kv]{key-display-list}
  \begin{syntax}
    key-display-list = \marg{key_1}\marg{display_1}\marg{key_2}\marg{display_2}...
  \end{syntax}
统一设置对应的级别。级别可以超过 5 个。当 \opt{key-display-list} 给出时，总是使用此选项设置的值，而忽略 \opt{key1} 等设置的值。
...
\subsection{\texttt{fmulticol/framed=tcbox}}\label{sec:fmulticol/framed=tcbox}

本库提供为多栏文字的外框提供了 \opt{tcbox} 可选值。表示用 \pkg{tcolorbox} 宏包的 
\tn{tcbox} 作为盒子外框。\keyref[fmulticol]{framed-options} 可以使用 \pkg{tcolorbox}
宏包提供的选项。
...
\opt{kv-usage} 用于将命令的索引格式切换至 cindex 的 \texttt{ikv} 格式。
\opt{reverse-list} 用于反转命令列表的显示位置，默认 \texttt{oneside} 文档显示在右边，\texttt{twoside} 文档，奇数页在右边，偶数页在左边。
...
\veta{module} 有几个特殊的值 \texttt{hook point}、\texttt{socket point}、\texttt{color name}，
分别用于标记此命令为钩子、代码槽、颜色名称。使用这几个值时最好也将 \opt{type} 也改为相同的值。
...
\begin{function}{\changes}
  \begin{syntax}
    \V\changes \marg{版本} \marg{日期} \marg{内容}
    \V\changes \oarg{小标题} \marg{版本} \marg{日期} \marg{内容}
  \end{syntax}
设置版本历史信息。\meta{版本}一般以 \texttt{v} 开头。\meta{日期} 为 \texttt{yyyy-mm-dd} 或 \texttt{yyyy/mm/dd}。
\meta{小标题} 若不设置，则为 \cs{generalname} 的值，必须非空，且只能包含纯文字。
\end{function}

\begin{function}{\generalname,\removegeneralname,\changesitemtitle}
\cs{changes} 的小标题。不能为空，且只能包含纯文字。\cs{removegeneralname} 用于删除它。
\cs{changesitemtitle} 用于设置小标题的格式，可重定义它以修改小标题的样式。
\end{function}

\begin{function}{\cus@doc@get@hypertarget,\cus_doc_get_hyper_target:nnN,\cus_doc_get_hyper_target:eeN}
  \begin{syntax}
    \V{\cus_doc_get_hyper_target:nnN} \marg{cs name} <{type}> \meta{cmd}
  \end{syntax}
把控制序列 \meta{cs name} 的引用名（即 \tn{label} 的参数）保存到 \meta{cmd} 中。
\meta{type} 为类型，一般为 \texttt{function}，也可为 \texttt{keyval}、\texttt{environment}、
\texttt{hook point}、\texttt{socket point}、\texttt{color name} 等。
\end{function}
...
\subsection{\texttt{fmulticol/framed=lfbox}}\label{sec:fmulticol/framed=lfbox}

本库提供为多栏文字的外框提供了了 \opt{lfbox} 可选值。表示用 \pkg{longfbox} 宏包的 
\tn{lfbox} 作为盒子外框。\keyref[fmulticol]{framed-options} 可以使用 \pkg{longfbox} 
宏包提供的选项。
...
\section{\pkg{framedmulticol}}

见\cref{sec:fmulticol}。
...
\begin{function}[EXP,added=2025/10/10]{\ekeys_cmd_arg_spec:N}
  \begin{syntax}
    \V*{\ekeys_cmd_arg_spec:N} \meta{ekeys-cmd}
  \end{syntax}
获得 \meta{ekeys-cmd} 的参数类型。如果该命令未定义或不是 \veta{ekeys-cmd} 则为 \texttt{X}。
\end{function}
...
\CHToI{latex}{\tn{@input@}};
\CHToI{latex}{\cs{\cs{c__template_values_root_tl}..}};
