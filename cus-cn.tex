...
它们会自动查找 \tn{graphicspath} 设置的路径，且可以自动补全文件扩展名。
\end{function}
...
本库提供一些与 PDF 文件的特性相关的功能。它们只能在使用了 \cs{DocumentMetadata} 之后使用。
...
如不设置，则使用 \tn{graphicspath} 设置的。
\end{keyval}

\begin{function}{\includegraphicspax}
  \begin{syntax}
    \V\includegraphics * \oarg{graphics options} \marg{filename}
  \end{syntax}
用此命令导入 PDF 文件可以保留超链接。
\end{function}
...
\meta{token} 或 \meta{\meta{balanced tokens}} 不在命令的参数里。
