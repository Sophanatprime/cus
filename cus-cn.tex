...
\newcommand{\hook}{\key[module=hook]}
\newcommand{\colna}{\key[module=color name]}
...
\begin{keyval}[path=multicolumns]{framed,framed-option,framed-option+}
  \begin{syntax}
    framed = <&fbox|...>
    framed-option = \marg{options}
  \end{syntax}
控制多栏文字整体用哪种盒子框住，多栏文字仍然可以分页。
\opt{framed-option} 为可能的选项。

默认仅提供 \opt{fbox} 这个可选值，表示用 \tn{fbox} 框住。其它库可能提供额外的值。
如 \cusmodule{box} 库提供 \opt{lfbox} 值等，见\cref{sec:multicolumns/framed=lfbox}。

使用 \opt{framed} 选项可能存在分页失败的情况。
\end{keyval}
...
宏包的 \tn{thispdfpagelabel} 命令，用于设置本页的 page label。
...
\LaTeX 默认的格式。这些命令是对 \csref{standardplaincombinedlist} 的简单封装，即
...
\csref{standardplaincombinedlist} 的第一个参数是显示的标题名称，第二个参数是目录类型。

\csref{multicolplaincombinedlist} 则是用于输出默认的多栏目录。可以使用一个可选参数
设置多栏的样式，见\cref{sec:multicol}，随后的两个参数和
\csref{standard\-plain\-combinedlist} 一样。
...
\toprule 
\thead{宏包}&\thead{\veta{type}}&\thead{\veta{level name}} & \thead{宏包}&\thead{\veta{type}}&\thead{\veta{level name}} \\ \midrule 
...
\begin{figure}[ht]\RawFloats
\begin{pagecenterbox}
\begin{minipage}{\dimeval{\paperwidth-3.4cm}}
\startmulticolumns[outer-sep=0pt,framed=fbox,ragged]
\newcommand{\ifinmiddle}[2]{\ifnum\tocthelevel=\tocthenextlevel\relax #1\else #2\fi}
\setlist[description]{nosep}
\tocsetstyle 
  {chapter,section}
...
\startmulticolumns[cols=4,ragged,outer-sep=0pt,framed=fbox,column-sep=10pt,
...
\startmulticolumns[2,ragged,outer-sep=0pt,framed=lfbox,
  adj-outer=-\dimeval{\paperwidth-3.4cm-\textwidth}]
\colorlet{tocgreen}{green!70!black}
\newcommand{\tochyperpage}{\toclink{\tocthepage}}
\hypersetup{hidelinks}
...
还可使用 \csref{LocalSpecifiedCombinedList} 来输出局部目录。
\csref{localspeci\-fiedtoc} 是一个特例，用来输出局部章节的目录。
...
\csref{LocalSpecifiedCombinedList} 目前还存在一点问题。\TODO
...
\tn{lfbox} 作为盒子外框。\keyref[multicolumns]{framed-option} 可以使用 \pkg{lfbox} 宏包 
提供的选项，但不能改变盒子的宽高和对齐方式。
