...
\ExplSyntaxOn
\cs_new:Npn \UseList { \cus_tl_use:nn }
...
\begin{description}
  ...
  \item[\texttt{space.hang}] 第一行额外的缩进；结果保存于 \tn{tmcbl@hang} dim 寄存器之中；默认为 \texttt{width.name} 的值；
  ...
  \item[\texttt{leader.content}] 设置引导线的内容；结果保存于 \tn{tmcbl@leadercontent} 宏之中；默认为西文句点；
  \item[\texttt{hyper.range}] 设置超链接要包含的内容，可选值为 \texttt{name}、\texttt{title}、\texttt{page} 和 \texttt{none}，可多选；最终结果保存于 \tn{tmcbl@hyperrange} int 寄存器之中，其中 \texttt{name} 将其加 1，\texttt{title} 将其加 2，\texttt{page} 将其加 4；默认为 \texttt{page}；
  ...
  \item[\texttt{code.hyper}] 将添加超链接要执行的代码替换为它，可使用 4 个参数，分别为 \texttt{name}、\texttt{title}、\texttt{hyper} 和 \texttt{page} 要执行的代码，结果保存在 \tn{tmcbl@hyper} 命令之中；
...
\begin{texnote}
只能以如下顺序执行上述三个命令：\cs{CUSLibraryDelayedUntil}、\cs{CUSDependency}、
\cs{CUSProvideExplLibrary}（或 \cs{CUSProvideLibrary}）。
...
目录（文件夹）的分隔符必须为 \texttt/，即是是在 Windows 系统上。
\end{function}
...
\begin{function}{\setplaintocdepth}
  \begin{syntax}
    \V\setplaintocdepth \marg{整数或层级名称}
  \end{syntax}
...
\begin{function}{\collectn_width:Nnnw,\collect_minipage:Nnnw,\collect_varwidth:Nnnw}
  \begin{syntax}
...
目前，在向后寻找可选参数时，会忽略空格，但未来可能会修改为不忽略空格，因此不宜在可选参数前加入空格（但必需参数 \texttt{m}、\texttt{r}、\texttt{R} 总忽略空格）。如 \verb*|\foo {a} [o]| 应写为 \verb|\foo {a}[o]|，假定 \veta{arg spec} 为 \texttt{mo}。
...
受影响的参数类型为 {\ttfamily\UseList{tTpPeEwW}{\textrm，}}。

使用 \texttt t、\texttt T、\texttt e、\texttt E、\texttt w、\texttt W 时，
设置 \texttt\& 将自动设置 \texttt\#。
...
执行效率从慢到块大致如下：
无 \texttt\# 且为 \texttt{tTwW} 的 \veta{ekeys-cmd} $<$
