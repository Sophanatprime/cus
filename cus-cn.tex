...
\meta{pos} 可选值为 \texttt{l}、\texttt{c}、\texttt{r}，
分别表示把 \meta{text} 放在盒子的左边、中间、右边，默认值为 \texttt{c}。
\meta{lap to} 的可选值为 \texttt{l}、\texttt{r}、\texttt{c}，
...
\meta{pos} 指定基线的位置，可选值为 \texttt{t}、\texttt{c}、\texttt{b}，
分别表示第一行文字的基线位置、文字的中间以及末行文字的基线。的默认值为 \texttt{c}。
若给出 \texttt{height}，则把文字放在高度为 \texttt{height} 的盒子中，
根据 \meta{inner-pos} 来决定文字的垂直位置，可选值为 \texttt{t}、\texttt{c}、\texttt{b}。
\end{function}
