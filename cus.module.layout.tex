...
\keys_define:nn { cus/layout }
  {
    ...
    layoutname .code:n = \__cus_set_layout_name:n {#1} ,
...
\cs_new:Npn \__cus_set_layout_name:n #1
  {
    ...
    \cs_if_exist:cTF { Gm@ \l__cus_papername_str }
      ...
      {
        \msg_warning:nnx { cus } { papername-unknown } { \l__cus_papername_str }
        \clist_put_right:Nn \g__cus_layout_geometry_clist
          { layoutwidth = 21cm , layoutheight = 29.7cm }
      }
  }
\use:e { \cs_new:Npn \exp_not:N \__cus_set_layout_name_aux:w #1 \token_to_str:N \Gm@setsize #2 (#3,#4) \c_left_brace_str #5 \c_right_brace_str #6 \s__cus_stop }
  {
    \clist_put_right:Nn \g__cus_layout_geometry_clist
      { layoutwidth = { #3#5 } , layoutheight = { #4#5 } }
  }
...
\NewDocumentCommand \setpagestyle { s m O{f@nch@fancyproto} +m }
  {
    \str_if_eq:eeTF {#2} {#3} 
      { \f@nch@pagestyle {#2} [f@nch@fancyproto] } { \f@nch@pagestyle {#2} [#3] } {#4}
    \IfBooleanF {#1} { \usepagestyle {#2} }
  }
