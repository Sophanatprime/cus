\CUSDependency{ package={
  (cus@TATE)lltjp-geometry, geometry, [usegeometry]typearea, fancyhdr
...
\cs_new_protected:Npn \cus_new_papername:nnn #1#2#3 % name, width, height
  {
    ...
    \cs_new_nopar:cpx { Gm@ #1 paper } ##1
      {
        \if_dim:w \l__cus_tmpa_dim > \l__cus_tmpb_dim 2 \else: 1 \fi:
...
\cs_new_protected:Npn \__cus_layout_marking_box:
  {
    \vbox_to_zero:n {
      ...
      \vb@xt@\z@{%
        \vskip\topmargin
        \ifodd\count\z@
          \let\@templeftskip\f@nch@O@olh \let\@temprightskip\f@nch@O@orh
        \else
          \let\@templeftskip\f@nch@O@elh \let\@temprightskip\f@nch@O@erh
        \fi
        ...
        \fi
        \ifodd\count\z@
          \let\@templeftskip\f@nch@O@olf \let\@temprightskip\f@nch@O@orf
        \else
          \let\@templeftskip\f@nch@O@elf \let\@temprightskip\f@nch@O@erf
        \fi
        \dim_set:Nn \@tempdima { \textwidth + \@templeftskip + \@temprightskip }
...
\NewDocumentCommand \setpagestyle { l m O{f@nch@fancyproto} +m }
  {
    \bool_lazy_or:nnTF 
      { \str_if_eq_p:ee { \c_tilde_str } { \exp_not:n {#3} } }
      { \str_if_eq_p:ee {#2} {#3} }
      { \f@nch@pagestyle {#2} [f@nch@fancyproto] } 
      { \f@nch@pagestyle {#2} [#3] } {#4}
    \str_case:nn {#1}
      {
        { * } { \usepagestyle{#2} }
        { . } { \usethispagestyle{#2} }
        { + } { \usepagestyle{#2} \usethispagestyle{#2} }
      }
  }
\cs_new_protected_nopar:Npn \copypagestyle #1#2 
  { 
    \cs_if_exist:cTF { ps@#2 }
      { 
        \cs_set_eq:cc { ps@#1 } { ps@#2 } 
        \cs_set_eq:cc { f@nch@ps@#1-is-fancyhdr } { f@nch@ps@#2-is-fancyhdr }
      }
      { \undefinedpagestyle{#2} }
  }
...
\cs_set:Npn \iftopfloat 
  { \if_meaning:w \topfloat \@empty \exp_after:wN \use_ii:nn \else: \use_i:nn \fi: }
\cs_set:Npn \ifbotfloat 
  { \if_meaning:w \botfloat \@empty \exp_after:wN \use_ii:nn \else: \use_i:nn \fi: }
