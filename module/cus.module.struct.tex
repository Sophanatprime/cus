    ...
    \cs_set:Npn \cus@toc@contentsline ##1##2##3##4##5##6##7##8##9
      {
        ...
        \tl_gset:No \tocthenumber { \use_ii:nnn ##5 }
        \tl_gset:No \tocthename { \use_iii:nnn ##5 }
...
\socket_new_plug:nnn { templatecbl } { by~name }
  {
    ...
    \group_begin:
      ...
      \tmcbl@pushdef \tmcblthetype
        {
          \bgroup \__cus_tmcbl_info:w ##3, \s__cus_mark
          \use:e { \UseInstance { templatecbl } { \tmcbltheclass } } 
            {##1}{##2}{##3}{##4}{##5}{##6}{##7}
        }
        { \egroup }
      \cus@tmcbl@getrange #5,,\@nil {#1}
    ...
    * .code = \__cus_tmcbl_edit_instance:n {#1} ,
    unknown .code:n = 
      \tl_if_head_eq_charcode:oNTF \l_keys_key_str *
        ...
        {
          \exp_args:Nno \IfInstanceExistTF { templatecbl } \l_keys_key_str
...
\cs_new_protected:Npn \__cus_tmcbl_edit_instance:n #1
  {
    \cus_map_heading_level:nn \tmcblthetype
      {
        \IfInstanceExistT { templatecbl } {##2}
...
\cs_new:Npn \cus@tmcbl@getrange #1, #2, #3 \@nil #4
  {
    \IfBlankTF{#1}{\def\@tempa{\CurrentTitleLevel}}
      {\edef\@tempa{\if\relax#1\relax \CurrentTitleLevel \else #1\fi}}
    \IfBlankTF{#2}{\def\@tempb{}}{\def\@tempb{#2}}%
    \expanded{\getcbllevelrange{#4}{\@tempa}[{\@tempb}]}{\tmcblthemin}{\tmcblthemax}
  }
...
\cs_new_protected:Npn \tmcbl@code@ #1#2#3#4#5#6#7
  {
    \bool_lazy_and:nnT { !\tmcbl@ignore }
      { \int_compare_p:nNn \c@tocdepth > {#4} }
      {
        { \tmcbl@before
          \mode_if_vertical:T 
          ...
          \null\nobreak % \tmcbl@skipifnz\hskip{-\leftskip}
          \tmcbl@skipifnz\hskip{-\tmcbl@hang}
...
\cs_new_protected:Npn \tmcbl@hyper@ #1#2#3#4 % name, title, leader, page
  {
    \int_case:nn { \tmcbl@hyperrange }
      {
        0 { #1#2#3#4 }
        1 { \tmcbl@hyperlink{#1}#2#3#4 }
        2 { #1\tmcbl@hyperlink{#2}#3#4 }
        3 { \tmcbl@hyperlink{#1#2}#3#4 }
        4 { #1#2#3\tmcbl@hyperlink{#4} }
        5 { \tmcbl@hyperlink{#1}#2#3\tmcbl@hyperlink{#4} }
        6 { #1\tmcbl@hyperlink{#2#3#4} }
        7 { \tmcbl@hyperlink{#1#2#3#4} }
      }
  }
\DeclareObjectType { templatecbl } { 7 }
...
\cs_new:Npn \__cus_tmcbl_hyper_range_none:  { \int_zero:N \tmcbl@hyperrange }
\cs_new_protected:Npn \__cus_tmcbl_fix_function:
  {
    \clist_remove_duplicates:N \tmcbl@hyperrangelist
    \int_zero:N \tmcbl@hyperrange
    \clist_map_function:NN \tmcbl@hyperrangelist \__cus_tmcbl_hyper_range:n
          ...
          \cus_gset_next_bookmark_text:n 
            { 
              \legacy_if:nT { BKM@numbered } 
                { \titleifname { \tl_use:c { titlethe #1 } ~ } { } } ##1 
            } ,
...
\def\chaptermark#1{\markboth{\MakeUppercase{\titleifname{\titlethechapter\ }{}#1}}{}}
\def\sectionmark#1{\markright{\MakeUppercase{\titleifname{\titlethesection\ }{}#1}}}
